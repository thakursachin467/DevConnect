    ...
    render() {
        ...
        const expItems = experience.map((exp) => (
            <li key={exp._id} className="list-group-item">
                <h4>{exp.company}</h4>
                <p>
                    <Moment format="DD/MM/YYYY">{exp.from}</Moment> to {exp.current ? <span>Now</span> : <Moment format="DD/MM/YYYY">{exp.to}</Moment>}
                </p>
        ...
        const eduItems = education.map((edu) => (
            <li key={edu._id} className="list-group-item">
                <h4>{edu.school}</h4>
                <p>
                    <Moment format="DD/MM/YYYY">{edu.from}</Moment> to {edu.current ? <span>Now</span> : <Moment format="DD/MM/YYYY">{edu.to}</Moment>}
                </p>
                <p><strong>Degree:</strong> {edu.degree} </p>
                <p><strong>Field Of Study:</strong> {edu.fieldofstudy} </p>
                <p>
                    {edu.location === '' || !edu.location ? null : <span><strong>Location:</strong> {edu.location}</span>}
                </p>
                <p>
